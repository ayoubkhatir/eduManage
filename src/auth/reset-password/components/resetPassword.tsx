...
export default function ResetPassword() {
  const { token, email } = useSearch({
    from: '/reset-password',
  }) as ResetPasswordSearch & { email?: string }
        ...
        {email && (
          <div className="rounded-xl bg-blue-50 p-4 mb-6 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800">
            <p className="text-sm text-blue-700 dark:text-blue-300">
              A reset request was submitted for <strong>{email}</strong>. If you
              received an email, follow its link to complete the reset. You can
              also set a new password here if you already have a valid token.
            </p>
          </div>
        )}
