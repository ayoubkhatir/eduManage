...
import {
  addTeacherServerFn,
  assignTeacherToClassAndSubjectServerFn,
  deleteTeacherAssignmentServerFn,
  deleteTeacherServerFn,
  editTeacherServerFn,
  getTeacherByIdServerFn,
  getTeacherByUserIdServerFn,
...
export function useDeleteTeacher() {
  ...
  return useMutation({
    mutationFn: (teacherId: string) =>
      deleteTeacherServerFn({ data: teacherId }),
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['teachers'] })
...
export const getTeacherQueryOptions = (
  args:
    | { fetchBy: 'userId'; userId: string }
    | { fetchBy: 'teacherId'; teacherId: string },
) => ({
  queryKey: [
    'teachers',
...
export function useDeleteTeacherAssignement() {
  ...
  return useMutation({
    mutationFn: async (assignmentId: string) =>
      deleteTeacherAssignmentServerFn({ data: assignmentId }),
    onSuccess: async (_, assignmentId) => {
      toast.success('Assignement deleted')
      await queryClient.invalidateQueries({
        queryKey: ['assignements', assignmentId],
      })
      await router.invalidate()
    },
  })
