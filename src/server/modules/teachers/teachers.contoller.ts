...
class TeachersController {
  ...
  async listTeachers({
    ...
    // Status filter
    if (normalizedStatus) {
      conditions.push(eq(teachersTable.status, normalizedStatus as StatusEnum))
    }
      ...
      conditions.push(
        exists(
          this.db
            ...
            .where(
              and(
                eq(teacherAssignmentsTable.teacherId, teachersTable.id),
                ilike(subjectsTable.name, `%${normalizedSubject}%`),
              ),
            ),
        ),
      )
    ...
    const data = rows.map(({ teacher, user }) =>
      TeacherUserDto(teacher, user, subjectsByTeacherId.get(teacher.id) ?? []),
    )
  ...
  async updateAssignment(
    ...
    input: Partial<
      Pick<
        AssignTeacherInput,
        'classId' | 'subjectId' | 'isPrimaryTeacher' | 'status'
      >
    >,
  ) {
  ...
  async getTeacherClassesDashboard(input: GetTeacherClassesType) {
    ...
    if (normalizedStatus && normalizedStatus !== 'All') {
      conditions.push(
        eq(teacherAssignmentsTable.status, normalizedStatus as any),
      )
    }
    ...
    const uniqueSubjectIds = new Set(
      allRowsForSummary.map((row) => row.subjectId),
    )
