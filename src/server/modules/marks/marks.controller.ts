  ...
  async getAssessmentMarks(assessmentId: string) {
    ...
    return {
      ...
      summary: {
        ...
        enteredMarks: entries.filter((e) => e.score !== null || e.absent)
          .length,
      },
    }
  }
  ...
  async saveStudentMarks(input: SaveStudentMarksSchema) {
    ...
    for (const mark of input.marks) {
      if (!validStudentSet.has(mark.studentId)) {
        throw new Error(
          `Student ${mark.studentId} does not belong to this class`,
        )
      }
    ...
    try {
      for (const mark of input.marks) {
        ...
        if (existingId) {
          ...
          const [createdMark] = await this.db
