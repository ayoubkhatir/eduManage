...
export const getAllStudentsServerFn = createServerFn({ method: 'GET' })
  ...
  .handler(async ({ data: search_queries }) => {
    const { data, pagination } =
      await studentsController.listStudents(search_queries)
...
export const deleteStudentServerFn = createServerFn({ method: 'POST' })
  ...
  .handler(
    async ({ data: studentId }) =>
      await studentsController.deleteStudent(studentId),
  )

export const getStudentsStatsServerFn = createServerFn({ method: 'GET' })
  .inputValidator(validCuidSchema)
  .handler(async ({ data: schoolId }) =>
    successResponse(await studentsController.getStudentsStats(schoolId)),
  )
...
export const getDashboardStatsServerFn = createServerFn({ method: 'GET' })
  .inputValidator(validCuidSchema)
  .handler(async ({ data: schoolId }) =>
    successResponse(await studentsController.getDashboardStats(schoolId)),
  )

export const getDashboardChartServerFn = createServerFn({ method: 'GET' })
  .inputValidator(dashboardPeriodSchema)
  .handler(async ({ data }) =>
    successResponse(await studentsController.getDashboardChart({ ...data })),
  )
