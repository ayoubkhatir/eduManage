...
class AuthController implements IAuthController {
  async login(input: LoginBody, headers: Headers) {
    try {
      ...
      const response = await authService.enrichUserWithInfo({
        ...data,
        token: accessToken,
      })
      return {
  ...
  async refresh(headers: Headers) {
    ...
    try {
      const userAgent = headers.get('user-agent') || ''
      const refreshResult = await authService.rotateSessionToken(
        currentRefreshToken,
        userAgent,
      )
  ...
  async forgotPassword(input: { email: string }) {
    try {
      ...
      return {
        success: true,
        message:
          'If an account exists with this email, you may proceed to reset the password',
        status: 200,
        data: {
          exists: !!user,
        },
  ...
  async fetchUserRoleData(userId: ID, role: UserRoleEnum) {
    switch (role) {
      ...
      case UserRoleEnum.TEACHER:
        ...
        const teacher = {
          subjects: assignments.map((ass) => ass.subject),
          ...restRawTeacher,
        }
