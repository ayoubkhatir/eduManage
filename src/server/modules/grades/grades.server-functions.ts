...
export const getAllGradesServerFn = createServerFn({ method: 'GET' })
  ...
  .handler(async ({ data: schoolId }) =>
    successResponse(await gradesController.getAllGrades(schoolId)),
  )

export const getAllGradesWithClassesAndSubjectsServerFn = createServerFn({
  method: 'GET',
})
  .inputValidator(validCuidSchema)
  .handler(async ({ data: schoolId }) =>
    successResponse(
      await gradesController.getAllGradesWithClassesAndSubjects(schoolId),
    ),
  )
...
export const addGradeServerFn = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data }) =>
    successResponse(await gradesController.addGrade(data)),
  )

export const deleteGradeServerFn = createServerFn({ method: 'POST' })
  .inputValidator(validCuidSchema)
  .handler(async ({ data: gradeId }) =>
    successResponse(await gradesController.deleteGrade(gradeId)),
  )
