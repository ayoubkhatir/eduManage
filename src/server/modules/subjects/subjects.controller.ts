...
class SubjectsController {
  ...
  async addSubject(data: AddSubjectSchema) {
    ...
    const missingGradeIds = data.gradeIds.filter(
      (id) => !existingGradeIds.has(id),
    )
  ...
  async listAllSubjects(schoolId: string) {
    ...
    const subjects: SubjectWithGrade[] = rawSubjects.map((s) => ({
      id: s.id,
      name: s.name,
      grades: s.gradeSubjects.map((s) => s.grade),
    }))
    return subjects
  }
