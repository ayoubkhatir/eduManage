...
export const getAllSchoolSubjectsServerFn = createServerFn({ method: 'GET' })
  ...
  .handler(
    async ({ data: schoolId }) =>
      successResponse(
        subjectsController.listAllSubjects(schoolId),
      ) as APIResponse<SubjectWithGrade[]>,
  )

export const addSubjectServerFn = createServerFn({ method: 'POST' })
  .inputValidator(addSubjectSchema)
  .handler(async ({ data }) =>
    successResponse(subjectsController.addSubject(data)),
  )
...
export const getStudentSubjectsServerFn = createServerFn({ method: 'GET' })
  ...
  .handler(async ({ data }) => {
    return successResponse(
      await subjectsController.listStudentSubjectsByUserId(data),
    )
  })
...
export const getTeacherSubjectsServerFn = createServerFn({ method: 'GET' })
  ...
  .handler(async ({ data }) => {
    return successResponse(
      await subjectsController.listTeacherSubjectsByUserId(data),
    )
  })
