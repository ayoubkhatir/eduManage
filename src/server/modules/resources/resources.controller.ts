...
class ResourcesController {
  ...
  async listResources(input: GetResourcesSchema): Promise<{
    data: ResourceDto[]
    pagination: {
      totalCount: number
      totalPages: number
    ...
    if (input.studentId) {
      ...
      // Resources visible to their specific class or subject-wide (null classId)
            conditions.push(
                or(
                    eq(resourcesTable.classId, student.class.id),
                    isNull(resourcesTable.classId),
                )!,
            )
