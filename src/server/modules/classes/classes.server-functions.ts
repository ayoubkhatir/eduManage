...
export const getClassesByGradeIdSevrerFn = createServerFn({ method: 'GET' })
  ...
  .handler(({ data: gradeId }) =>
    successResponse(classesController.listClassesByGradeId(gradeId)),
  )

export const getAllClassesServerFn = createServerFn({ method: 'GET' })
  .inputValidator(validCuidSchema)
  .handler(({ data: schoolId }) =>
    successResponse(classesController.listClasses(schoolId)),
  )
...
export const getClassesByTeacherUserIdServerFn = createServerFn({
  method: 'GET',
})
  .inputValidator(validCuidSchema)
  .handler(async ({ data: teacherUserId }) =>
    successResponse(classesController.getClassesByTeacherUserId(teacherUserId)),
  )

export const deleteClassByIdServerFn = createServerFn({ method: 'POST' })
  .inputValidator(validCuidSchema)
  .handler(({ data: classId }) =>
    successResponse(classesController.deleteClassById(classId)),
  )
